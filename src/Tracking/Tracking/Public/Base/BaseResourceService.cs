...
public class BaseResourceService
{
    ...
    {
        if (response.StatusCode != HttpStatusCode.OK) throw BuildAfterShipException(response);
    ...
    public static T ProcessData<T>(Response response)
    {
        ...
        try
        {
            var dataJson = ToJson(FromJson<BaseResponse>(response.Content).Data);
    ...
    {
        if (adapter == null) return ProcessData<T>(response);

        T obj;
        try
        {
            var source = ProcessData<S>(response);
    ...
    private static AfterShipError BuildAfterShipException(Response response)
    {
        ...
        int.TryParse(meta["code"]?.ToString(), out var code);
