...
public class Authenticator
{
    ...
    public void Sign(HttpRequestMessage request)
    {
        if (AuthenticationType != API_KEY)
        {
            ...
            var h = CanonicalHeader(tmpHeaders);
            string rs;
            try
            {
            ...
            {
                if (request.Content != null) request.Content.CopyToAsync(ms).Wait();
                bodyBytes = ms.ToArray();
            }

            var bodyString = Encoding.UTF8.GetString(bodyBytes);
            var s = SignString(request.Method.ToString(), bodyString, request.Headers.GetValues("date").First(), h, rs);
            if (AuthenticationType == AUTH_TYPE_AES)
            {
                var aesSign = Encryption.HmacSha256Encrypt(s, ApiSecret);
                tmpHeaders.TryAddWithoutValidation("as-signature-hmac-sha256", aesSign);
            }
            else if (AuthenticationType == AUTH_TYPE_RSA)
            {
                string rsaSign;
                try
                {
                    rsaSign = Encryption.RsaPssSha256Encrypt(s, ApiSecret);
                }
                catch (Exception e)
                {
    ...
    public static string CanonicalHeader(HttpHeaders headers)
    {
        ...
        {
            var key = h.Key.Trim();
            var value = string.Join(",", h.Value).Trim();
    ...
    public static string CanonicalResource(HttpRequestMessage request)
    {
        ...
        foreach (string? key in queryParams.Keys)
        {
            ...
            if (key != null && value != null) sortedParams[key] = value;
        }
    ...
    public static string SignString(string menthod, string body, string date, string canonicalHeader,
